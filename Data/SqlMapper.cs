...
namespace Stacy.Core.Data
{
	...
	public static class SqlMapper
	{
		...
		public static IEnumerable<Tuple<string, string, int>> GetCachedSQL(int ignoreHitCountAbove = int.MaxValue)
		{
			IEnumerable<Tuple<string, string, int>> data =
				_queryCache.Select(pair => Tuple.Create<string, string, int>(pair.Key.connectionString, pair.Key.sql, pair.Value.GetHitCount()));
	...
	/// </summary>
		                                               ...
		                                               CommandType? commandType)
		{
			...
			using (IDbCommand cmd = SetupCommand(cnn, transaction, sql, info.ParamReader, param, commandTimeout, commandType))
			{
				IDataReader reader = null;
			    try
			    {
			        ...
			        {
			            tuple =
			                info.Deserializer = new DeserializerState(hash, GetDeserializer(typeof (T), reader, 0, -1, false));
		...
		public static IEnumerable<TReturn> Query<TFirst, TSecond, TThird, TFourth, TFifth, TReturn>(this IDbConnection cnn,
		                                                                                            ...
		                                                                                            Func
			                                                                                            ...
			                                                                                            TFourth, TFifth, TReturn>
			                                                                                            map, dynamic param = null,
		                                                                                            ...
		                                                                                            CommandType? commandType =
			                                                                                            null)
		{
			return MultiMap<TFirst, TSecond, TThird, TFourth, TFifth, TReturn>(cnn, sql, map, param as object, transaction,
			                                                                   buffered, splitOn, commandTimeout, commandType);
		}
#endif
		...
		{
			IEnumerable<TReturn> results = MultiMapImpl<TFirst, TSecond, TThird, TFourth, TFifth, TReturn>(cnn, sql, map, param,
			                                                                                               transaction, splitOn,
			                                                                                               commandTimeout,
			                                                                                               commandType, null,
			                                                                                               null);
			...
			int? commandTimeout, CommandType? commandType, IDataReader reader, Identity identity)
		{
			...
			try
			{
				    ...
				    hash != deserializer.Hash)
				{
					Func<IDataReader, object>[] deserializers =
						GenerateDeserializers(
							new[] {typeof (TFirst), typeof (TSecond), typeof (TThird), typeof (TFourth), typeof (TFifth)}, splitOn, reader);
				...
				Func<IDataReader, TReturn> mapIt =
					GenerateMapper<TFirst, TSecond, TThird, TFourth, TFifth, TReturn>(deserializer.Func, otherDeserializers, map);
		...
		private static Func<IDataReader, object>[] GenerateDeserializers(Type[] types, string splitOn, IDataReader reader)
		{
			...
			Func<Type, int> nextSplit = type =>
				{
					string currentSplit = splits[splitIndex];
...
#if !CSHARP30
		private class FastExpando : DynamicObject, IDictionary<string, object>
		{
...
#if !CSHARP30
		private static Func<IDataReader, object> GetDynamicDeserializer(IDataRecord reader, int startBound, int length,
		                                                                bool returnNullIfFirstMissing)
#else
		...
		private static int ExecuteCommand(IDbConnection cnn, IDbTransaction transaction, string sql,
		                                  ...
		                                  CommandType? commandType)
		{
			using (IDbCommand cmd = SetupCommand(cnn, transaction, sql, paramReader, obj, commandTimeout, commandType))
			{
...
#pragma warning restore 618
						...
						else
						{
							...
							else
							{
								...
								else
								{
									...
									il.EmitCall(OpCodes.Call, typeof (Convert).GetMethod("ChangeType", new[] {typeof (object), typeof (Type)}),
	...
	public sealed class DefaultTypeMap : SqlMapper.ITypeMap
	{
		...
		{
			PropertyInfo property = _properties.FirstOrDefault(p => string.Equals(p.Name, columnName, StringComparison.Ordinal))
			                        ??
