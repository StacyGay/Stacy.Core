...
namespace Stacy.Core.Data
{
		...
		where T : new()
	{
		...
		{
			Data = data as List<T> ?? data.ToList();
			PrepData(Data);
		...
		public BulkInsert<T> ExecuteMerge(IEnumerable<string> keys, bool delete = false, string deleteParam = "")
		{
			...
			try
			{
				...
				Connection.Execute(mergeQuery, transaction: trans, commandTimeout: 3000);
				trans.Commit();
			}
			catch (Exception)
			{
		...
		private void PrepData(IEnumerable<T> data)
		{
			...
			foreach (var item in data)
			{
				...
				{
					if(((DateTime)prop.GetValue(item)) < new DateTime(1900,1,1))
						prop.SetValue(item, new DateTime(1900,01,01));
				}
			}
		}
	}
