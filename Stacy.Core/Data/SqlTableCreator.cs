...
namespace Stacy.Core.Data
{
	public class SqlTableCreator
	{
		...
		public List<ColumnDetails> CreateFromSqlTable(string tableName)
		{
			...
			const string sql =
                @"SELECT DISTINCT c.COLUMN_NAME as Name, c.COLUMN_DEFAULT as DefaultValue, 
                    ...
                    ISNULL(c.NUMERIC_PRECISION, 0) as NumericPrecision, ISNULL(c.NUMERIC_SCALE, 0) as NumericScale
		...
		public static string GetCreateFromColumnDetailSQL(string tableName, List<ColumnDetails> tableColumns)
		{
			string sql = "CREATE TABLE [" + tableName + "] (\n";
			// columns
			foreach (ColumnDetails column in tableColumns)
			{
				sql += "[" + column.Name + "] " + column.DataType;
                if (column.CharLength > 0)
                    sql += "(" + column.CharLength.ToString() + ") ";
                else if (column.NumericPrecision > 0 && column.NumericScale > 0)
                    sql += "(" + column.NumericPrecision + "," + column.NumericScale + ") ";
                else if (column.CharLength == -1)
                    sql += "(MAX) ";
				if (column.IsIdentity)
					sql += " IDENTITY (0,1) ";
				else if (column.DefaultValue != null && column.DefaultValue.Length > 0)
